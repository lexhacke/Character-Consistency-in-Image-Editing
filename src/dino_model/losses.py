...
class FocalLossFromLogits(nn.Module):
    ...
    def forward(self, y, yhat):
        """
        Expects y, yhat of shape B, C, H, W
        """
        P = torch.sigmoid(yhat)
        focal_weight = (
            y * self.alpha * (1 - P) ** self.gamma
            + (1 - y) * (1 - self.alpha) * P ** self.gamma
        )
...
class DiceLossFromLogits(nn.Module):
    ...
    def forward(self, y, yhat):
        ...
        p_sum = P.sum(dim=(1, 2, 3))
        loss = 1 - (2 * yP + self.smooth) / (y_sum + p_sum + self.smooth)
        return loss.mean()
